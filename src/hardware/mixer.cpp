...
static std::array<int16_t, MIXER_BUFSIZE> queue_buffer;

static void MIXER_QueueAudio(uint16_t len)
{
	assert(len * 2 <= queue_buffer.size());
	...
	while (reduce--) {
		...
		queue_buffer[buf_idx++] = MIXER_CLIP(sample);
	...
	const uint32_t size = len * MIXER_SSIZE;
...
// Map latency to SDL sample count
static const std::map<int, int> latency_samples{
        {5, 256}, {10, 512}, {20, 1024}, {50, 2048}, {100, 4096}};
...
void MIXER_Init(Section* sec) {
	...
	auto blocksize = static_cast<uint16_t>(latency_samples.at(mixer.latency));
